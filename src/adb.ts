...
export class AdbService {
  ...
  async uiDump(): Promise<UiNode[]> {
    await this.ensureConnected();
    let xml = "";
    const remotePath = `/data/local/tmp/remote-ui-${Date.now()}.xml`;
    try {
      ...
      if (!xml.includes("<hierarchy")) {
        throw new Error("UI_DUMP_STDOUT_UNSUPPORTED");
      }
    } catch {
      try {
        await this.execute([
          "-s", this.target, "shell", "uiautomator", "dump", remotePath,
        ]);
        const result = await this.execute([
          "-s", this.target, "exec-out", "cat", remotePath,
        ]);
        xml = String(result.stdout);
      } finally {
        try {
          await this.execute(["-s", this.target, "shell", "rm", "-f", remotePath]);
        } catch {
          // A limpeza do arquivo temporário é apenas preventiva.
        }
      }
    }
    if (!xml.includes("<hierarchy")) {
      throw new Error("UI_DUMP_FAILED: a TV não devolveu a estrutura da tela.");
    }
    const nodes: UiNode[] = [];
    const regex =
      /<node\b[^>]*>/g;
